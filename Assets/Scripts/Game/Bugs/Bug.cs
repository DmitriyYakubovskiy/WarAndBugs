...
public abstract class Bug : Entity
{
    ...
    public override void TakeDamage(float damage, bool bonus=true)
    {
        ...
        if (lives <= 0)
        {
            if (player != null && bonus==true)
            {
                ...
                player.GetComponent<Player>().Money += (int)(money + money/100f * SaveSystem.Data.coefficientOfDifficulty*10f);
                ...
                for (int i = 0; i < value; i++)
                {
                    ...
                    postion.z = 0;
                    var buf = Instantiate(expGameObject, entityTransform.position + postion, entityTransform.rotation);
    ...
    {
        return Mathf.Sqrt(((playerPosition.x - entityTransform.position.x) * (playerPosition.x - entityTransform.position.x)) + ((playerPosition.y - entityTransform.position.y) * (playerPosition.x - entityTransform.position.y)));
    }
    ...
    protected virtual void AiLogics()
    {
        ...
        if (randomMove && randomMoveTime <= 0)
        {
            ...
            var ThisPosition = entityTransform.position;
