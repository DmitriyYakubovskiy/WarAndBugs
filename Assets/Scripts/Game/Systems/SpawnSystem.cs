...
public class SpawnSystem : MonoBehaviour
{
    ...
    public int aliveBugs=0;
    public int levelBugs=1;
    ...
    private void Start()
    {
        ...
        RangeLeft = rangeLeft;
        startTime /= SaveSystem.Data.coefficientOfDifficulty;
        Debug.Log($"Time: {startTime}");
    }

    private void Update()
    {
        if (aliveBugs > 100 * levelBugs)
        {
            startTime *= 2;
            levelBugs += 1;
            aliveBugs = 0;
            messagesManager.ShowMessage($"Bugs have a level now: ", levelBugs);
            Debug.Log($"У жуков теперь уровень: {levelBugs}");
            Debug.Log($"Time: {startTime}");
        }
    ...
    private void SpawnBoss()
    {
        ...
        messagesManager.ShowMessage($"The boss has appeared!");
    }
