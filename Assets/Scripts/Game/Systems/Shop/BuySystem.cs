...
public class BuySystem : MonoBehaviour
{
    ...
    public void UpdateGunButtons()
    {
        ...
        for (int i = 0; i < gunItems.Length; i++)
        {
            ...
            if (SaveSystem.Data.ShopItemNames[gunItems[i].NameItem] == 1)
            {
                gunItems[i].SetButtonSelectInteractable(true);
                gunItems[i].SetButtonBuyInteractable(false);
                if (SaveSystem.Data.SelectedItemNames["selectedGun"] == gunItems[i].NameItem) gunItems[i].SetButtonSelectInteractable(false);
            }
            else
            {
    ...
    public void UpdateOtherButtons()
    {
        ...
        for (int i = 0; i < droneItems.Length; i++)
        {
            ...
            if (SaveSystem.Data.ShopItemNames[droneItems[i].NameItem] >= 1)
            {
                ...
                if (SaveSystem.Data.SelectedItemNames[droneItems[i].SelectedItemName] == droneItems[i].NameItem) droneItems[i].SetButtonSelectInteractable(false);
            }
            else
            {
