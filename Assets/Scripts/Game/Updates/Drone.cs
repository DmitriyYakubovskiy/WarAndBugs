...
public class Drone : Sound
{
    ...
    private void Start()
    {
        if (SaveSystem.Data.ShopItemNames[nameDrone] == 0 || SaveSystem.Data.SelectedItemNames["selectedDrone"] != nameDrone) gameObject.SetActive(false);

        int level = SaveSystem.Data.ShopItemNames[nameDrone];
        ...
        foreach (Transform child in droneTransform.GetComponentsInChildren<Transform>())
        {
    ...
    private void Move()
    {
        ...
        droneRigidbody.AddForce(moveVector,ForceMode2D.Force);
    }
    ...
    private void AiLogics()
    {
        if (player != null)
        {
            ...
            var ThisPosition = droneTransform.position;
